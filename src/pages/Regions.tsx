...
const Regions = () => {
  ...
  const [selectedRegion, setSelectedRegion] = useState<EnhancedRegion | null>(
    null,
  );
  ...
  useEffect(() => {
    if (refreshTrigger > 0) {
      console.log(
        `Refreshing regions due to trigger change (${refreshTrigger})...`,
      );
  ...
  const handleOpenRegionDialog = useCallback(
  ...
  const handleOpenAdminDialog = useCallback(
    (region: EnhancedRegion, method: "new" | "existing" = "existing") => {
      console.log(
        "Opening admin dialog for region:",
        region.name,
        "Method:",
        method,
      );
