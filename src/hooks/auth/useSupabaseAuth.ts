...
export const useSupabaseAuth = (supabaseClient: any = supabase, initialSession = null): UseSupabaseAuthReturn => {
 ...
 const handleSignIn = useCallback(async (email: string, password: string) => {
   ...
   try {
     ...
     // İlk öncə istifadəçi rolunu əldə edək
     const { data: roleData, error: roleError } = await supabase
       .from('user_roles')
       .select('role, region_id, sector_id, school_id')
     ...
     // Sonra istifadəçi ilə JWT-yə rol məlumatlarını əlavə edirik
     ...
     if (updateError) {
       console.error('İstifadəçi metadata yeniləmə xətası:', updateError);
     }
