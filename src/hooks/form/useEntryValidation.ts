...
export const useEntryValidation = () => {
  ...
  const { validateField: baseValidateField } = useValidation();
  ...
  const validateField = useCallback((value: string, column: Column): string[] => {
    ...
    if (column.validation) {
      const validationResults = baseValidateField(value, column.validation);
  ...
  const validateAllFields = useCallback((
    entries: Array<{columnId: string; value: string}>, 
