...
export const useColumnForm = ({ column, categoryId, onSave }: UseColumnFormProps) => {
  ...
  useEffect(() => {
    if (column) {
      console.log('Updating form with column data:', column);
      form.reset({
        name: column.name,
        type: column.type,
      ...
      console.log('Setting category_id for new column:', categoryId);
  ...
  const removeOption = (optionToRemove: ColumnOption) => {
    console.log('Removing option:', optionToRemove);
  ...
  const onSubmit = async (values: ColumnFormValues) => {
    ...
    try {
      ...
      const columnData = {
        ...
        ...(isEditMode && column ? { id: column.id } : {})
      };

      console.log('Calling onSave with data:', columnData);
