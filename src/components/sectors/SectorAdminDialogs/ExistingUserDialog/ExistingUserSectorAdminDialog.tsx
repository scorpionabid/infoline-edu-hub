...
const ExistingUserSectorAdminDialog = ({
  ...
  sectorName,
  onSuccess,
}: ExistingUserSectorAdminDialogProps) => {
  ...
  }, [userId, sectorId, t, onSuccess, onClose]);

  // Filter users based on search term (filtering is now handled by the database function)
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{t("assign_sector_admin") || "Sektor admin təyin et"}</DialogTitle>
          <DialogDescription>
            {t("select_user_for_sector") || "Zəhmət olmasa bu sektor üçün istifadəçi seçin:"}  
