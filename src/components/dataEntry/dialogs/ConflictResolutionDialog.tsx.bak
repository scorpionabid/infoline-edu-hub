...
interface ConflictResolutionDialogProps {
  ...
  onResolve: (resolution: 'local' | 'server' | 'merge' | 'field-by-field', fieldResolutions?: Record<string, 'local' | 'server'>) => void;
  onCancel?: () => void;
  userInfo?: {
    currentUser: string;
    conflictUser: string;
...
export const ConflictResolutionDialog: React.FC<ConflictResolutionDialogProps> = ({
  ...
  const handleFieldResolutionChange = useCallback((columnId: string, resolution: 'local' | 'server') => {
  ...
  const getResolutionColor = (strategy: ResolutionStrategy): string => {
    switch (strategy) {
      ...
      case 'server': return 'bg-green-50 border-green-200';
      case 'merge': return 'bg-purple-50 border-purple-200';
      case 'field-by-field': return 'bg-orange-50 border-orange-200';
      default: return 'bg-gray-50 border-gray-200';
    }
  };

  const renderValue = (value: any, label: string, timestamp?: string, user?: string) => (
  ...
  const renderConflictField = (conflict: ConflictData) => (
    <div key={conflict.columnId} className="space-y-4 p-4 border rounded-lg">
      <div className="flex items-center justify-between">
        <h4 className="font-medium">{conflict.columnName || conflict.columnId}</h4>
        {selectedStrategy === 'field-by-field' && (
          <div className="flex gap-2">
            <Button
              size="sm"
              variant={fieldResolutions[conflict.columnId] === 'local' ? 'default' : 'outline'}
              onClick={() => handleFieldResolutionChange(conflict.columnId, 'local')}
            >
              {t('useLocal')}
            </Button>
            <Button
              size="sm"
              variant={fieldResolutions[conflict.columnId] === 'server' ? 'default' : 'outline'}
              onClick={() => handleFieldResolutionChange(conflict.columnId, 'server')}
            >
              {t('useServer')}
            </Button>
          </div>
