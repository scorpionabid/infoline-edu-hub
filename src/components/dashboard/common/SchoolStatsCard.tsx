...
const SchoolStatsCard: React.FC<SchoolStatsCardProps> = ({ stats }) => {
  ...
  const totalCompletionRate =
    stats.length > 0
      ? stats.reduce((sum, s) => sum + (s.completionRate || 0), 0) /
        stats.length
      : 0;

  const pendingForms = stats.reduce(
    (sum, s) => sum + (s.pendingForms || s.pendingCount || 0),
    0,
  );
  const completedForms = stats.reduce(
    (sum, s) => sum + (s.completedForms || 0),
    0,
  );
        ...
        <div className="space-y-4">
          ...
          <div className="pt-2">
            ...
            <p className="text-2xl font-bold">
              {Math.round(totalCompletionRate)}%
            </p>
          </div>
